    ...
    public static void Start()
    {
        lock (_lock)
        {
            ...
            if (!File.Exists(workerPath))
                throw new FileNotFoundException("worker.py was not found.", workerPath);

            if (!File.Exists(startupPath))
                throw new FileNotFoundException("startup.py was not found.", startupPath);
            ...
            _proc = Process.Start(psi)
                ?? throw new InvalidOperationException("Failed to start the Python worker process.");
    ...
    private static object Send(Dictionary<string, object?> payload)
    {
        lock (_lock)
        {
            ...
            if (string.IsNullOrWhiteSpace(line))
                return "Python worker returned no response.";
            ...
            if (!ok)
            {
                if (root.TryGetProperty("error", out JsonElement errEl))
                    return $"Python error: {errEl.GetString()}";
...
private static string GetPythonPath()
{
    ...
    private static object? NormalizeScalar(object? value)
    {
        ...
        return value switch
        {
            ...
            decimal m => m,
            bool b => b,
            string s when s.StartsWith(PythonObjectReferencePrefix, StringComparison.Ordinal) =>
                new Dictionary<string, object?>
                {
                    ["__pyexcel_arg_type"] = "pyobj",
                    ["name"] = s.Substring(PythonObjectReferencePrefix.Length)
                },
            string s => s,
    ...
    private static bool TryCreateNumericSetPayload(string name, object value, out Dictionary<string, object?>? payload, out string? file)
    {
        ...
        file = Path.Combine(Path.GetTempPath(), $"pyexcel_set_numeric_{Guid.NewGuid():N}.bin");
        ...
        if (value is object[,] range)
        {
            ...
            for (int r = r0; r <= r1; r++)
            {
                for (int c = c0; c <= c1; c++)
                {
                    ...
                    if (cell is ExcelEmpty or ExcelMissing || cell is null)
                        values[k++] = double.NaN;
                    else if (cell is double x)
                        values[k++] = x;
                    else if (cell is int i)
                        values[k++] = i;
                    else if (cell is long l)
                        values[k++] = l;
                    else if (cell is decimal m)
                        values[k++] = (double)m;
                    else
                        return false;
                }
    ...
    private static bool TryCreateTableSetPayload(string name, object value, bool hasHeaders, out Dictionary<string, object?>? payload, out List<string>? files)
    {
        ...
        if (value is not object[,] range)
            return false;

        int r0 = range.GetLowerBound(0), r1 = range.GetUpperBound(0);
        int c0 = range.GetLowerBound(1), c1 = range.GetUpperBound(1);
        int cols = c1 - c0 + 1;
        int dataStart = hasHeaders ? r0 + 1 : r0;
        int rows = r1 - dataStart + 1;
        if (rows < 0) rows = 0;
        ...
        for (int c = c0; c <= c1; c++)
        {
            ...
            for (int r = dataStart; r <= r1; r++)
            {
                ...
                {
                    nums[r - dataStart] = (double)m;
                    vals.Add((double)m);
                }
                else
                {
                    numeric = false;
                    vals.Add(NormalizeScalar(cell));
                }
            }

            if (numeric)
            {
                string file = Path.Combine(Path.GetTempPath(), $"pyexcel_set_table_{Guid.NewGuid():N}.bin");
                byte[] bytes = new byte[nums.Length * sizeof(double)];
                Buffer.BlockCopy(nums, 0, bytes, 0, bytes.Length);
                File.WriteAllBytes(file, bytes);
                files.Add(file);
                columns.Add(new Dictionary<string, object?> { ["name"] = colName, ["type"] = "numeric", ["file"] = file, ["na"] = "NaN" });
            }
            else
            {
                columns.Add(new Dictionary<string, object?> { ["name"] = colName, ["type"] = "character", ["values"] = vals });
            }
        }
    ...
    private static object ConvertNumericTransferToExcel(JsonElement el)
    {
        ...
        double[] data = ReadDoubleFile(file, rows * cols);
    ...
    private static object ConvertTableTransferToExcel(JsonElement el)
    {
        ...
        int offset = includeHeaders ? 1 : 0;
        var output = new object[rows + offset, cols];
        JsonElement columns = el.GetProperty("columns");
        for (int c = 0; c < cols; c++)
        {
            JsonElement col = columns[c];
            string colName = col.GetProperty("name").GetString() ?? string.Empty;
            string colType = col.GetProperty("type").GetString() ?? "character";
            if (includeHeaders) output[0, c] = colName;
            if (string.Equals(colType, "numeric", StringComparison.OrdinalIgnoreCase))
            {
                string file = col.GetProperty("file").GetString() ?? string.Empty;
                double[] data = ReadDoubleFile(file, rows);
                for (int r = 0; r < rows; r++) output[r + offset, c] = double.IsNaN(data[r]) ? ExcelEmpty.Value : data[r];
                TryDeleteFile(file);
            }
            else
            {
                JsonElement values = col.GetProperty("values");
                for (int r = 0; r < rows; r++) output[r + offset, c] = ConvertJsonToExcel(values[r]);
            }
        }
        return output;
    }
